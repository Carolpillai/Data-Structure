...
class StackGUI:
    ...
    def create_widgets(self):
        ...
        self.title_label = tk.Label(self.root, text="Stack Operations", font=("Helvetica", 20, "bold"),
                                    bg="#C2185B", fg="#FFFFFF")  # Deep Rose
        self.title_label.pack(pady=20)

        self.entry_frame = tk.Frame(self.root, bg="#FFD1DC")
        self.entry_frame.pack(pady=10)
        
        self.entry_label = tk.Label(self.entry_frame, text="Enter Element:", font=("Helvetica", 14),
                                    bg="#FFD1DC", fg="#000000")
        ...
        self.button_frame = tk.Frame(self.root, bg="#FADADD")
        ...
        self.insert_button = tk.Button(self.button_frame, text="Insert (Push)", command=self.insert_element,
                                       bg=pastel_colors[0], **self.button_style)
        self.insert_button.grid(row=0, column=0, padx=10, pady=5)

        self.delete_button = tk.Button(self.button_frame, text="Delete (Pop)", command=self.delete_element,
                                       bg=pastel_colors[1], **self.button_style)
        self.delete_button.grid(row=0, column=1, padx=10, pady=5)

        self.peek_button = tk.Button(self.button_frame, text="Peek", command=self.peek_element,
                                     bg=pastel_colors[2], **self.button_style)
        self.peek_button.grid(row=1, column=0, padx=10, pady=5)

        self.empty_button = tk.Button(self.button_frame, text="Is Empty", command=self.check_if_empty,
                                      bg=pastel_colors[3], **self.button_style)
        self.empty_button.grid(row=1, column=1, padx=10, pady=5)

        self.size_button = tk.Button(self.button_frame, text="Get Size", command=self.get_size,
                                     bg=pastel_colors[4], **self.button_style)
        self.size_button.grid(row=2, column=0, padx=10, pady=5)

        self.traverse_button = tk.Button(self.button_frame, text="Traversal (Display Stack)", command=self.traverse_stack,
                                         bg=pastel_colors[5], **self.button_style)
        ...
        self.quit_button = tk.Button(self.button_frame, text="Quit", command=self.root.quit,
                                     bg="#FFC1CC", **self.button_style)  # Light pink quit
        self.quit_button.grid(row=3, column=0, columnspan=2, pady=10)

        self.result_display = tk.Text(self.root, height=12, width=50, state=tk.DISABLED,
                                      font=("Helvetica", 12), bg="#FFF0F5", fg="#000000", wrap=tk.WORD)
